...
class Commodity:
    ...
    def __init__(self, csv_name):
        ...
        #print(self.Y)
        #print(dataset.info())
       ##################Kmeans for unlabelled data######################
       
       
        myKMC=KMeans(n_clusters=2)
        myKMC.fit(dataset.drop('WPI',axis=1))
        
        print(myKMC.cluster_centers_)
        
        dataset['Cluster']=dataset['Month'].apply(lambda x:1 if x>=6 else 0 )
        print(dataset.head(20))
        
        from sklearn.metrics import confusion_matrix,classification_report
        print(confusion_matrix(dataset['Cluster'],myKMC.labels_))
        print(classification_report(dataset['Cluster'],myKMC.labels_))
        sns.lmplot(x='Year',y='WPI',data=dataset,fit_reg=False,height=6,aspect=1,hue='Cluster')
